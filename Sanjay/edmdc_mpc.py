...
def observables(x_std, scaler):
    """
    Lifted observables for EDMDc — must match training exactly.
    x_std: standardized 10-state vector.
    """
...
def precompute_ref_std(ref_traj, scaler, n_states=10):
    """Build standardized reference with position AND velocity."""
    T = len(ref_traj)
...
def main():
    ...
    # drop psi_des from U if 4 columns
    if U_all.shape[2] == 4:
        U_all = U_all[:, :, :3]
    ...
    t_ref   = t_all[run_idx]
    X_true  = states_all[run_idx]        # (T, 10)
    U_saved = U_all[run_idx]             # (T, 3)
    ...
    # --- precompute reference in standardized space ---
    ...
    # --------------------------------------------------------
    # EDMD-MPC closed-loop trajectory tracking
    # Feed [thrust, phi_des, theta_des] to inner PID
    # Re-lift from true state every step
    # --------------------------------------------------------
    X_mpc = np.zeros((T, 10))
    U_mpc = np.zeros((T, 3))

    # start from true initial 12-state
    x_current_12 = np.zeros(12)
    x10_init     = X_true[0]
    x_current_12[0:6]  = x10_init[0:6]    # pos, vel
    x_current_12[6:8]  = x10_init[6:8]    # phi, theta
    # psi(8) = 0
    x_current_12[9:11] = x10_init[8:10]   # p, q
    # r(11) = 0
    ...
    for k in range(T - 1):
        if k % 50 == 0:
            print(f"  step {k}/{T-1}  pos=({X_mpc[k,0]:.2f}, "
                  f"{X_mpc[k,1]:.2f}, {X_mpc[k,2]:.2f})")

        # lift current 10-state to observable space
        ...
        # execute on real sim via inner PID
    ...
    print(f"\nSolve time — avg: {1e3*np.mean(solve_times):.2f} ms  "
          f"max: {1e3*np.max(solve_times):.2f} ms")

    # --------------------------------------------------------
    # METRICS
    # --------------------------------------------------------
    pos_err_mpc  = np.linalg.norm(X_mpc[:, 0:3]  - ref_xyz, axis=1)
    pos_err_true = np.linalg.norm(X_true[:, 0:3] - ref_xyz, axis=1)

    print(f"\nPosition RMSE — EDMD-MPC: {rmse(X_mpc[:,0:3], ref_xyz):.4f} m")
    print(f"Position RMSE — Saved PID: {rmse(X_true[:,0:3], ref_xyz):.4f} m")
    print(f"Mean pos error — EDMD-MPC: {np.mean(pos_err_mpc):.4f} m")
    print(f"Mean pos error — Saved PID: {np.mean(pos_err_true):.4f} m")
    ...
    ax.plot(X_true[:,0],   X_true[:,1],   X_true[:,2],
            "gray", lw=1.5, label="Saved PID")
    ax.plot(X_mpc[:,0],    X_mpc[:,1],    X_mpc[:,2],
            "g-", lw=1.5, label="EDMD-MPC")
    ax.set_xlabel("x [m]"); ax.set_ylabel("y [m]"); ax.set_zlabel("z [m]")
    ax.set_title("EDMD-MPC trajectory tracking")
    ...
    n_cols = 5
    n_rows = 2
    fig2, axs = plt.subplots(n_rows, n_cols, figsize=(20, 7))
    for i in range(10):
        row, col = divmod(i, n_cols)
        ax = axs[row, col]
        ax.plot(t_ref, X_true[:, i], "gray", lw=1.2, label="Saved PID")
        ax.plot(t_ref, X_mpc[:, i],  "g-",   lw=1.2, label="EDMD-MPC")
    ...
    for i in range(3):
        ...
        axs3[i].plot(t_ref, U_mpc[:, i],   "g-",   lw=1.2, label="EDMD-MPC")
    ...
    plt.plot(t_ref, pos_err_mpc,  "g",    label="EDMD-MPC")
    plt.xlabel("t [s]")
