...
def run_mpc(mpc, sim, target, scaler, steps, lifted, label):
    ...
    for k in range(steps - 1):
        ...
        z = lifted_state_from_x(state, scaler) if lifted else scaler.transform(state.reshape(1, -1)).flatten()
...
def main():
    ...
    train_indices = [i for i in range(states_all.shape[0]) if i not in {39, 59, 129}]
    A_lin, B_lin, _, x_lin_scaler, u_lin_scaler = fit_linear_baseline(t_all, states_all, U_all, train_indices)
    ...
    for plot_i, (name, target) in enumerate(cases, start=1):
        ...
        X_lin, U_lin, cap_lin, st_lin = run_mpc(
            build_linear_mpc(A_lin, B_lin, x_lin_scaler, u_lin_scaler, sim_lin),
