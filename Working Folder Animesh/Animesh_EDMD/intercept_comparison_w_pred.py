...
def build_target_ref_traj_lead(target, t_now, dt, N, lead_time):
    """
    Build an MPC reference that aims at a future target point.
    The same lead point is repeated across the horizon.
    """
    t_hit = t_now + lead_time
    pos = target.position(t_hit)
    vel = target.velocity(t_hit)
...
def run_pid_intercept(sim, target, x0_12, dt, t_max, capture_radius):
    ...
    for k in range(n_max - 1):
        t_now = k * dt
        ref_k = build_target_ref_pid(target, t_now + LEAD_TIME)
...
def run_edmdc_mpc_intercept(mpc, sim, scaler, target, x0_12, dt, N,
    ...
    for k in range(n_max - 1):
        ...
        ref_traj_k = build_target_ref_traj_lead(target, t_now, dt, N, LEAD_TIME)
