...
n_obs_test = len(observables(np.zeros(12), scaler))
...
def rolling_horizon_rmse(states, inputs, A, B, scaler, u_scaler, horizon, observables_fn):
    ...
    for start in range(n_windows):
        ...
        x_pred = scaler.inverse_transform(psi_pred[:12, :].T)
...
print("\n========== B ROW NORMS ==========")
labels_12 = ['x','y','z','vx','vy','vz','phi','theta','psi','p','q','r']
for i, lbl in enumerate(labels_12):
...
lifted_labels = ['sin_phi','cos_phi','sin_theta','cos_theta',
                 ...
                 'sin_psi','cos_psi','psi*r','psi²',
                 'bias']
for i, lbl in enumerate(lifted_labels):
    print(f"  {lbl:>12s}: {np.linalg.norm(B[i+12,:]):.6f}")
print("==================================")
...
for test_idx in test_indices:
    ...
    n_cols = 4
    n_rows = 3
    fig, axs = plt.subplots(n_rows, n_cols, figsize=(20, ))
...
model_data = {
    ...
    "lambda": best_lam,
    "state_labels": labels,
    "u_labels": ["thrust", "phi_des", "theta_des","psi_des"],
    "observable_labels": labels_12 + lifted_labels,
